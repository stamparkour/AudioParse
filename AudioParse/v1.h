...
//AUDIO PARSE V1
//https://www.desmos.com/calculator/kdemoo7njd
//pitch + timing
...
void AddNoteBuffer(uint8_t value, float start) {
	for (int i = 0; i < sizeof(buffer) / sizeof(buffer[0]); i++) {
		if (buffer[i].note == 0) {
			if (i == depthTarget && start - buffer[i].start > 0) {
				notes.push_back(Note{ (uint8_t)0, start - buffer[i].start });
			}
...
void RemoveNoteBuffer(uint8_t value, float end) {
	for (int i = 0; i < sizeof(buffer) / sizeof(buffer[0]); i++) {
		if (buffer[i].note == value) {

			if (i == depthTarget) {
				notes.push_back(Note{ (uint8_t)value, end - buffer[i].start });
			}
			buffer[i].note = 0;
			buffer[i].start = end;
			return;
...
int parseMidiMessage(uint8_t status, uint8_t* k) {
	...
	case 0x8:
		if (low == channelTarget) {
			RemoveNoteBuffer(k[0], totalTime);
	...
	case 0x9:
		if (low == channelTarget) {
			if (k[1] == 0) { //velocity is zero
				RemoveNoteBuffer(k[0], totalTime);
			}
			else {
				AddNoteBuffer(k[0], totalTime);
			}
		}
		if (low > maxChannel) maxChannel = low;
		return 2;
...
void parseTrack(MidiChunk& chunk) {
	...
	notes.push_back(Note{ (uint8_t)buffer[depthTarget].note, totalTime - buffer[depthTarget].start });
}
