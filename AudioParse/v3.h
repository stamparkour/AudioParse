...
//AUDIO PARSE V3
//https://www.desmos.com/calculator/dur3bg7azk
//pitch + timing + volume + velocity
...
struct Note {
	...
	Note(int midiNote, float length, int volume, int velocity) {
		this->length = length;
		this->volume = volume / 128.0f;
		this->velocity = velocity / 128.0f;
		HZ = toHZ(midiNote);
	}
	...
	int Append(char* str, size_t s, float& time, float& hz, float& volume) {
		...
		float h = velocity;
		volume = this->volume;
		return sprintf_s(str, s, "%g,%.1f,%.3f,%.3f", t, j, k, h);
	}
};
...
void AddNoteBuffer(uint8_t value, uint8_t volume, uint8_t velocity , float start) {
	for (int i = 0; i < sizeof(buffer) / sizeof(buffer[0]); i++) {
		if (buffer[i].note == 0) {
			if (i == depthTarget && start - buffer[i].start > 0) {
				notes.push_back(Note{ (uint8_t)0, start - buffer[i].start, buffer[i].volume, buffer[i].velocity });
			}
...
void RemoveNoteBuffer(uint8_t value, uint8_t volume, uint8_t velocity, float end) {
	for (int i = 0; i < sizeof(buffer) / sizeof(buffer[0]); i++) {
		if (buffer[i].note == value) {

			if (i == depthTarget) {
				notes.push_back(Note{ (uint8_t)value, end - buffer[i].start, buffer[i].volume, buffer[i].velocity });
			}
			buffer[i].note = 0;
			buffer[i].volume = volume;
			buffer[i].velocity = velocity;
...
int parseMidiMessage(uint8_t status, uint8_t* k) {
	...
	case 0x8:
		if (low == channelTarget) {
			RemoveNoteBuffer(k[0], currentVolume, k[1], totalTime);
	...
	case 0x9:
		if (low == channelTarget) {
			if (k[1] == 0) { //volume is zero
				...
				AddNoteBuffer(k[0], currentVolume, k[1], totalTime);
			}
		}
		if (low > maxChannel) maxChannel = low;
		return 2;
